...
class AdminCartOverviewItem(BaseModel):
    user_id: int
    username: str
    email: str
    total_items: int   # 购物车中商品种类数
    total_quantity: int # 总数量

class AdminCartOverviewResponse(BaseModel):
    items: list[AdminCartOverviewItem]
    total: int
    page: int
    size: int
    pages: int   

class GlobalCartStats(BaseModel):
    total_quantity: int