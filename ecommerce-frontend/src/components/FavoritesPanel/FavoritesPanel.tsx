...
const FavoritesPanel = ({ embedded = false, showTitle = true }: FavoritesPanelProps) => {
    ...
    return (
      <div className="text-center py-5">
        ...
        <Link
          to="/"
          className="btn mt-2"
          style={{ background: 'var(--primary-container)', color: 'var(--on-primary-container)', borderRadius: 'var(--radius)', fontWeight: 600 }}
        >
          去逛逛
        </Link>
      </div>
  ...
  return (
    <div>
      {showTitle && (
        <h3 className="fw-bold mb-4" style={{ fontFamily: '"Manrope","Noto Sans SC",sans-serif', fontSize: embedded ? '1.35rem' : '1.5rem' }}>
          <i className="bi bi-heart-fill me-2" style={{ color: '#e74c3c' }} />
          我的收藏
        </h3>
      )}
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: embedded ? 'repeat(auto-fill, minmax(220px, 220px))' : 'repeat(auto-fill, minmax(220px, 220px))',
          gap: '1.25rem',
