              ...
              <div className={styles.orderBody}>
                ...
                <div className={styles.timeline}>
                  {timeline.map((node) => (
                    <div
                      key={node.key}
                      className={`${styles.timelineItem} ${node.done ? styles.timelineDone : ''}`}
                    >
              ...
              <div className={styles.orderFooter}>
                ...
                <div className="d-flex gap-2 flex-wrap">
                  {order.status === 'pending' && (
                    <>
                      <button className="btn btn-sm btn-outline-danger" onClick={() => handleCancel(order.id)}>取消订单</button>
                      <button className="btn btn-sm btn-danger" onClick={() => handleGoPay(order)}>去支付</button>
                    </>
                  )}
                  {['paid', 'shipped', 'completed'].includes(order.status) && order.after_sale_status !== 'requested' && (
                    <button className="btn btn-sm btn-outline-warning" onClick={() => handleAfterSale(order.id)}>申请售后</button>
                  )}
                  {(order.status === 'completed' || order.status === 'cancelled') && (
                    <button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(order.id)}>删除订单</button>
                  )}
                  <button
